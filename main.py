...
tk.Button(card,
          ...
          bg="#6C63FF",
          fg="white",
          activebackground="#5a54d1",
          activeforeground="white",
          bd=0,
          width=12,
          height=1,
          command=lambda: show("difficulty")
          ).pack(pady=25)

tk.Label(card,
         text="AI-based Cognitive & Habit Analysis",
         font=("Segoe UI", 9),
         fg="gray",
         bg="white").pack()
...
def loading_screen():
    ...
    root.after(2000, analyze)

# -------------------------------
# DIFFICULTY
# -------------------------------
tk.Label(difficulty_frame, text="Select Difficulty",
         font=("Segoe UI", 16,"bold"), bg=bg).pack(pady=40)
...
tk.Button(difficulty_frame, text="Easy",
          command=lambda: set_difficulty("easy")).pack(pady=15)

tk.Button(difficulty_frame, text="Medium",
          command=lambda: set_difficulty("medium")).pack(pady=15)

tk.Button(difficulty_frame, text="Hard",
          command=lambda: set_difficulty("hard")).pack(pady=15)

# -------------------------------
# GAME
# -------------------------------
sequence_label = tk.Label(game_frame,
...
def play_level():
    ...
    if level == 3:
        ...
        if difficulty == "hard":
            # mix colors + numbers
            numbers = [random.choice(colors_list + [str(random.randint(1, 9))]) for _ in range(5)]
        else:
            numbers = [random.choice(colors_list) for _ in range(4)]

        sequence_label.config(text="  ".join(numbers))

        if difficulty == "hard":
            root.after(2000, lambda: sequence_label.config(text="(Enter Reverse: colors + numbers)"))
        else:
            root.after(2000, lambda: sequence_label.config(text="Type color names"))
...
def check():
    global level, score, q_index

    user = entry.get().upper().split()

    # Hard mode reverse logic
    if difficulty == "hard":
        correct_list = numbers[::-1]
    else:
        correct_list = numbers
...
def analyze():
    ...
    card = tk.Frame(result_frame, bg="white", bd=0, highlightthickness=0)
    card.config(highlightbackground="#d3d3d3", highlightcolor="#d3d3d3", highlightthickness=1)
